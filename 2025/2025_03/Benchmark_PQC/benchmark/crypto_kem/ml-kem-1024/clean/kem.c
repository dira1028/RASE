...
int benchmark()
{
    ...
    uint8_t(*pk)[PQCLEAN_MLKEM1024_CLEAN_CRYPTO_PUBLICKEYBYTES] = malloc(100 * sizeof(*pk));
    uint8_t(*sk)[PQCLEAN_MLKEM1024_CLEAN_CRYPTO_SECRETKEYBYTES] = malloc(100 * sizeof(*sk));
    uint8_t(*ct)[PQCLEAN_MLKEM1024_CLEAN_CRYPTO_CIPHERTEXTBYTES] = malloc(100 * sizeof(*ct));
    uint8_t(*ss)[PQCLEAN_MLKEM1024_CLEAN_CRYPTO_BYTES] = malloc(100 * sizeof(*ss));
    ...
    for (int i = 0; i < 1000; i++)
    {
        ...
        for (int j = 0; j < 100; j++)
        {
            if (PQCLEAN_MLKEM1024_CLEAN_crypto_kem_keypair(pk[j], sk[j], &keygen) == -1)
            {
                printf("Error in PQCLEAN_MLKEM1024_CLEAN_crypto_kem_keypair\n");
                return -1;
            }
        }
        ...
        for (int j = 0; j < 100; j++)
        {
            if (PQCLEAN_MLKEM1024_CLEAN_crypto_kem_enc(ct[j], ss[j], pk[j], &enc) == -1)
            {
                printf("Error in PQCLEAN_MLKEM1024_CLEAN_crypto_kem_enc\n");
                return -1;
            }
        }
        ...
        for (int j = 0; j < 100; j++)
        {
            if (PQCLEAN_MLKEM1024_CLEAN_crypto_kem_dec(ss[j], ct[j], sk[j], &dec, &gen_secretkey) == -1)
            {
                printf("Error in PQCLEAN_MLKEM1024_CLEAN_crypto_kem_dec\n");
                return -1;
            }
        }

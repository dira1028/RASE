...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk, double *keygen)
{
    ...
    *keygen += (end.tv_sec - start.tv_sec) * 1000.0+ (end.tv_nsec - start.tv_nsec) / 1e6;
    return 0;
}
...
int benchmark()
{
    ...
    printf("---benchmark sphincs shake-256s---\n");
    ...
    for (int j = 0; j < 100; j++)
    {
        ...
        keypair += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_nsec - t1.tv_nsec) / 1e6;
        sign += (t3.tv_sec - t2.tv_sec) * 1000.0 + (t3.tv_nsec - t2.tv_nsec) / 1e6;
        vrfy += (t4.tv_sec - t3.tv_sec) * 1000.0 + (t4.tv_nsec - t3.tv_nsec) / 1e6;
    }
    printf("keygen : %f\n", keygen / 10000);
    printf("keypair: %f\n", keypair / 10000);
    printf("sign: %f\n", sign / 10000);

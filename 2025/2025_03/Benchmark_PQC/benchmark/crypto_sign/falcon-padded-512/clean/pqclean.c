    ...
    uint8_t *pk, uint8_t *sk, double *keygen, double *key_encode)
{
    ...
    *keygen += (double)(tB.tv_sec - tA.tv_sec) * 1000 + (tB.tv_nsec - tA.tv_nsec) / 1e6;
    *key_encode += (double)(tD.tv_sec - tC.tv_sec) * 1000 + (tD.tv_nsec - tC.tv_nsec) / 1e6;
    return 0;
}
        ...
        const uint8_t *m, size_t mlen, const uint8_t *sk, double *decode_s, double *hash_to_point, double *sign, double *encode_s)
{
    ...
    for (;;)
    {
        ...
        *hash_to_point += (double)(tD.tv_sec - tC.tv_sec) * 1000 + (tD.tv_nsec - tC.tv_nsec) / 1e6;
    ...
    const uint8_t *m, size_t mlen, const uint8_t *pk, double *decode_v, double *hash_to_point_v, double *vrfy)
{
    ...
    if (PQCLEAN_FALCONPADDED512_CLEAN_modq_decode(h, 9,
                                                  pk + 1, PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES - 1) != PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES - 1)
    {
        return -1;
    }
    ...
    const uint8_t *sm, size_t smlen, const uint8_t *pk, double *decode_v, double *hash_to_point_v, double *vrfy)
{
    ...
    if (do_verify(sm + 1, sigbuf, sigbuflen,
                  sm + PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_BYTES, pmlen, pk, decode_v, hash_to_point_v, vrfy) < 0)
    {
        return -1;
    }
...
int benchmark()
{
    uint8_t pk[100][PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES];
    uint8_t sk[100][PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_SECRETKEYBYTES];
    uint8_t m[1000];
    uint8_t sm[100][1000 + PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_BYTES];
    size_t smlen[100];
    size_t mlen = 1000;
    ...
    printf("=========benchmark FALCONPADDED512=========\n");
    for (int j = 0; j < 1000; j++)
    {
        clock_gettime(CLOCK_MONOTONIC, &t1);
        for (int i = 0; i < 100; i++)
        {
            if (PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_keypair(pk[i], sk[i], &keygen, &key_encode) == -1)
            {
                printf("Error in PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_keypair\n");
                return -1;
            }
        }
        ...
        for (int i = 0; i < 100; i++)
        {
            if (PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign(sm[i], &smlen[i], m, mlen, sk[i], &decode_s, &hash_to_point_s, &sign, &encode_s) == -1)
            {
                printf("Error in PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign\n");
            }
        }
        clock_gettime(CLOCK_MONOTONIC, &t3);
        for (int i = 0; i < 100; i++)
        {
            if (PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_open(m, &mlen, sm[i], smlen[i], pk[i], &decode_v, &hash_to_point_v, &vrfy) == -1)
            {
                printf("Error in PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_verify\n");
                return -1;
            }
        }

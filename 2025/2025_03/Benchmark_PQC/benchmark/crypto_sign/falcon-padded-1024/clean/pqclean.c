...
int PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_keypair(
    uint8_t *pk, uint8_t *sk, double *keygen, double *encode_k)
{
    union
    {
    ...
    *encode_k += (tD.tv_sec - tC.tv_sec) * 1000 + (tD.tv_nsec - tC.tv_nsec) / 1e6;
    return 0;
}
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t sigbuflen,
        const uint8_t *m, size_t mlen, const uint8_t *sk, double *decode_s, double *hash_to_point_s, double *sign, double *encode_s)
{
    union
    {
        uint8_t b[72 * 1024];
    ...
    for (;;)
    {
        ...
        *decode_s += (double)(tB.tv_sec - tA.tv_sec) * 1000 + (tB.tv_nsec - tA.tv_nsec) / 1e6;
        *hash_to_point_s += (double)(tD.tv_sec - tC.tv_sec) * 1000 + (tD.tv_nsec - tC.tv_nsec) / 1e6;
        *sign += (double)(tF.tv_sec - tE.tv_sec) * 1000 + (tF.tv_nsec - tE.tv_nsec) / 1e6;
        *encode_s += (double)(tG.tv_sec - tF.tv_sec) * 1000 + (tG.tv_nsec - tF.tv_nsec) / 1e6;
        if (v != 0)
        {
            inner_shake256_ctx_release(&sc);
...
do_verify(
    ...
    const uint8_t *m, size_t mlen, const uint8_t *pk, double *decode_v, double *hash_to_point_v, double *vrfy)
{
    union
    {
        uint8_t b[2 * 1024];
    ...
    if (PQCLEAN_FALCONPADDED1024_CLEAN_modq_decode(h, 10,
                                                   pk + 1, PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_PUBLICKEYBYTES - 1) != PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_PUBLICKEYBYTES - 1)
    {
        return -1;
    }
    ...
    const uint8_t *m, size_t mlen, const uint8_t *sk, double *decode_s, double *hash_to_point_s, double *sign, double *encode_s)
{
    ...
    if (do_sign(sig + 1, sig + 1 + NONCELEN, vlen, m, mlen, sk, decode_s, hash_to_point_s, sign, encode_s) < 0)
    {
        return -1;
    }
    sig[0] = 0x30 + 10;
    ...
    const uint8_t *m, size_t mlen, const uint8_t *sk, double *decode_s, double *hash_to_point_s, double *sign, double *encode_s)
{
    ...
    if (do_sign(sm + 1, sigbuf, sigbuflen, m, mlen, sk, decode_s, hash_to_point_s, sign, encode_s) < 0)
    {
        return -1;
    }
    sm[0] = 0x30 + 10;
    ...
    const uint8_t *sm, size_t smlen, const uint8_t *pk, double *decode_v, double *hash_to_point_v, double *vrfy)
{
    ...
    if (do_verify(sm + 1, sigbuf, sigbuflen,
                  sm + PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_BYTES, pmlen, pk, decode_v, hash_to_point_v, vrfy) < 0)
    {
        return -1;
    }
...
int benchmark()
{
    uint8_t pk[100][PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_PUBLICKEYBYTES];
    uint8_t sk[100][PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_SECRETKEYBYTES];
    uint8_t m[1000];
    uint8_t sm[100][1000 + PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_BYTES];
    size_t smlen[100];
    size_t mlen = 1000;
    ...
    printf("=========benchmark FALCONPADDED1024=========\n");
    for (int j = 0; j < 1000; j++)
    {
        clock_gettime(CLOCK_MONOTONIC, &t1);
        for (int i = 0; i < 100; i++)
        {
            if (PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_keypair(pk[i], sk[i], &keygen, &key_encode) == -1)
            {
                printf("Error in PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_keypair\n");
                return -1;
            }
        }
        ...
        for (int i = 0; i < 100; i++)
        {
            if (PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign(sm[i], &smlen[i], m, mlen, sk[i], &decode_s, &hash_to_point_s, &sign, &encode_s) == -1)
            {
                printf("Error in PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign\n");
            }
        }
        clock_gettime(CLOCK_MONOTONIC, &t3);
        for (int i = 0; i < 100; i++)
        {
            if (PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_open(m, &mlen, sm[i], smlen[i], pk[i], &decode_v, &hash_to_point_v, &vrfy) == -1)
            {
                printf("Error in PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_verify\n");
                return -1;
            }
        }

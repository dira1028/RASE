 ...
 * Format pk: [PUB_SEED || root]
 */
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk,double*keypare) {
    ...
    /*--------------------시간측정-------------------------*/
    *keypare += (t2.tv_sec - t1.tv_sec) * 1000 + (t2.tv_nsec - t1.tv_nsec) / 1e6;
    return 0;
}
...
int benchmark()
{
    ...
    printf("---benchmark sphincs sha 128f---\n");
